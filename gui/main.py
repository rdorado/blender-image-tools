...
from core import Collection, Surface, Vector
...
class MainWindow(QMainWindow):
    ...
    *****************************************************************'''
    def create_menu(self):
        self.menu = {"&File": {
              "&Open": self.add_collection,
              "&Acquire": self.acquire,
              "&Close": self.app_exit
           }}
        app_menu = self.menuBar()
        for (menu_text, menu_items) in self.menu.items():
            menu = app_menu.addMenu(menu_text)
            for (menu_item_text, action) in menu_items.items():
                menu_action = QAction(menu_item_text, self)
        ...
        #add_collection_action = QAction("&Exit", self)
        #add_collection_action.triggered.connect(self.app_exit)
        #file_menu.addAction(add_collection_action)
