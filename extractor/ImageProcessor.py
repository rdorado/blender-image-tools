...
from ImageProcessorMainWindow import Ui_ImageProcessorMainWindow
from ImageProcessorDialogs import CustomDialog
from SurfaceEditor import SurfaceEditorMainWindow
from Preprocess import cluster, segmentate, find_segments_borders, get_surfaces, borders_to_image, mask_to_color, image_grey_to_color
from core import GraphicObject, Point, PointSequence, Surface
from utils import QPixmap_to_nparray, nparray_to_QPixmap
'''
try:
...
class Canvas(QLabel):
    ...
    def __init__(self, config, img, *args, **kwargs):
    ...
    def setImage(self, img):
        if len(self.current_image) != 0: self.backward_history.append(self.current_image)
        self.current_image = img
        pixmap = nparray_to_QPixmap(self.current_image)
        pixmap = pixmap.scaled(self.scale * pixmap.size())
    ...
    def repaintCurrentPixmap(self):
        ...
        pixmap = pixmap.scaled(self.scale*width, self.scale*height)
        super().setPixmap(pixmap)
    ...
    def mousePressEvent(self, e):
        ...
        if e.button() == Qt.LeftButton:
            ...
            if action is not None:
                ...
                #img = self.current_image[self.origin_pos.y():self.last_pos.y(), self.origin_pos.x():self.last_pos.x(), :]
                img = self.current_image.copy()
                img = img[self.origin_pos.y():self.last_pos.y(), self.origin_pos.x():self.last_pos.x(), :]
                img = np.ascontiguousarray(img)
                #print(img.flags)
                #pixmap = self.current_pixmap.copy(self.origin_pos.x(),self.origin_pos.y(), self.last_pos.x(), self.last_pos.y())
                self.command_from_canvas.emit({"command":"create_new_document_from_selection", "selection": img})
                #print(action.text())
...
class Document:
    ...
    def __init__(self, name, config, image):
        self.name = name
        self.canvas = Canvas(config, image)
        self.widget =  None
...
class ImageProcessorMainWindow(Ui_ImageProcessorMainWindow):


    new_doc_seq = 1

    def __init__(self, *args, **kwargs):
        super(ImageProcessorMainWindow, self).__init__(*args, **kwargs)
        #self.setupUi(self)
        self.show()
        self.documents = []
        self.config = Config()
        ...
        self.actionApplyCluster.triggered.connect(self.apply_cluster)
        self.actionAcquireSurfaces.triggered.connect(self.acquire_surfaces)
        self.actionSegmentate.triggered.connect(self.segmentate)
        self.actionFindBorders.triggered.connect(self.find_segments_borders)
        '''
    ...
    def open_file(self):
        ...
        if path:
            ...
            self.add_document(img, filename)
            




    def add_document(self, img, name=None):
        if name == None:
            name = "Document "+str(self.new_doc_seq)
            self.new_doc_seq += 1

        # Set new document with pixmap and name
        new_document = Document(name, self.config, img)
...
'''
    ...
    def segmentate(self):
        indx = self.documentTabs.currentIndex()
        #pixmap = self.documents[indx].canvas.pixmap()
        arr = self.documents[indx].canvas.current_image;
        
        # QPixmap to np array
        #logging.debug("[ImageProcessor] Transforming to np array")
        #arr = QPixmap_to_nparray(pixmap)
        #logging.debug("[ImageProcessor] Segmenting...")
        (clusters, mask) = segmentate(arr)
        logging.debug("[ImageProcessor] Transforming to image")
        cvImg = image_grey_to_color(mask)
        #cvImg = mask_to_color(mask)
        logging.debug("[ImageProcessor] Transformed to image")
        # np array to QPixmap
        logging.debug("[ImageProcessor] np array to QPixmap")
        #pixmap = nparray_to_QPixmap(cvImg)
        self.documents[indx].canvas.setImage(cvImg)


    def find_segments_borders(self):
        indx = self.documentTabs.currentIndex()
        arr = self.documents[indx].canvas.current_image;
        ...
        logging.debug("Transforming to image")
        (h, w, channels) = arr.shape
        cvImg = borders_to_image(border_clusters, h, w)
        logging.debug("Transformed to image")
        # np array to QPixmap
        logging.debug("np array to QPixmap")
        pixmap = nparray_to_QPixmap(cvImg)
        self.documents[indx].canvas.setImage(cvImg)


    def acquire_surfaces(self):
        logging.debug("Segmenting")
        indx = self.documentTabs.currentIndex()
        arr = self.documents[indx].canvas.current_image;
        ...
        logging.debug("Segmenting")
        (clusters, mask) = segmentate(arr)
        logging.debug("Finding borders")
        border_clusters = find_segments_borders(clusters, mask)
        logging.debug("Finding surfaces")
        pobj = get_surfaces(border_clusters)
        
        logging.debug("Launching editor")
        self.editor = SurfaceEditorMainWindow()
    ...
    def apply_cluster(self):
        ...
        if dlg.exec():
            indx = self.documentTabs.currentIndex()
            n = int(dlg.lineEdit.text())
            pixmap = self.documents[indx].canvas.pixmap()

            # QPixmap to np array
            arr = QPixmap_to_nparray(pixmap)
            cvImg = cluster(arr, n)
            cvImg = image_grey_to_color(cvImg)
            
            # np array to QPixmap
            pixmap = nparray_to_QPixmap(cvImg)
            self.documents[indx].canvas.updatePixmap(pixmap)
        else:
            print("Cancel!")
...
if __name__ == '__main__':
    ...
    window = ImageProcessorMainWindow()
    app.exec_()